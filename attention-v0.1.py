...
embedding_dim = 5
embedding_weights = torch.nn.Embedding(len(vocab), embedding_dim)
embeddings = embedding_weights(tokenized_input)
print("\n============ Embeddings (1, seq_len, embed_dim) ============")
print(embeddings)

weight_dim = 4
W_q = torch.nn.Parameter(torch.randn(embedding_dim, weight_dim))
W_k = torch.nn.Parameter(torch.randn(embedding_dim, weight_dim))
W_v = torch.nn.Parameter(torch.randn(embedding_dim, weight_dim))
...
att_weights = torch.nn.functional.softmax(att_scores / math.sqrt(embedding_dim), dim=-1)
